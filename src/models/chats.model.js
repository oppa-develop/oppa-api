...
chatsModel.getChatsByUserId = async (user_id) => {
  const [chats] = await pool.query("SELECT chats.* FROM chats INNER JOIN users_has_chats ON users_has_chats.chats_chat_id = chats.chat_id WHERE users_user_id = ?;", [user_id]);

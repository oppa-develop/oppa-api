...
authModel.getClientByEmail = async (email) => {
  ...
  try {
    ...
    let [user] = await conn.query(`SELECT admin_id, client_id, provider_id, users.*, (SELECT total FROM wallet_movements INNER JOIN users ON users.user_id = users_user_id WHERE users.email = ? ORDER BY wallet_movements_id DESC LIMIT 1) AS 'credit' FROM users LEFT JOIN admins ON admins.users_user_id = user_id LEFT JOIN clients ON clients.users_user_id = user_id LEFT JOIN providers ON providers.users_user_id = user_id WHERE users.email = ?;`, [email, email]);
    if (user[0].client_id) {

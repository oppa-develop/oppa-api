...
usersModel.editUser = async (userData) => {
  if (userData.img_url === null) {
    await pool.query("UPDATE users SET firstname = ?, lastname = ?, gender = ?, phone = ?, birthdate = ?, updated_at = ? WHERE user_id = ?", [userData.firstname, userData.lastname, userData.gender, userData.phone, userData.birthdate, userData.updated_at, userData.user_id]);
  } else {
    await pool.query("UPDATE users SET firstname = ?, lastname = ?, gender = ?, phone = ?, birthdate = ?, updated_at = ?, img_url = ? WHERE user_id = ?", [userData.firstname, userData.lastname, userData.gender, userData.phone, userData.birthdate, userData.updated_at, userData.img_url, userData.user_id]);
  }
